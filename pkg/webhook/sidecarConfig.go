...
func validateAnnotations(annotations map[string]string) (err error) {
	for name, value := range annotations {
		if v, ok := annotationRegistry[name]; ok {
			...
			istiolog.Warnf("Potentially misspelled annotation '%s' with value '%s' encountered", name, value)
		}
	}
	return
}
...
func injectRequired(ignored []string, config *Config, podSpec *corev1.PodSpec, metadata *metav1.ObjectMeta) bool { // nolint: lll
	...
	if useDefault {
		for _, neverSelector := range config.NeverInjectSelector {
			...
			if err != nil {
				istiolog.Warnf("Invalid selector for NeverInjectSelector: %v (%v)", neverSelector, err)
			} else if !selector.Empty() && selector.Matches(labels.Set(metadata.Labels)) {
				istiolog.Debugf("Explicitly disabling injection for pod %s/%s due to pod labels matching NeverInjectSelector config map entry.",
	...
	if useDefault {
		for _, alwaysSelector := range config.AlwaysInjectSelector {
			...
			if err != nil {
				istiolog.Warnf("Invalid selector for AlwaysInjectSelector: %v (%v)", alwaysSelector, err)
			} else if !selector.Empty() && selector.Matches(labels.Set(metadata.Labels)) {
				istiolog.Debugf("Explicitly enabling injection for pod %s/%s due to pod labels matching AlwaysInjectSelector config map entry.",
					metadata.Namespace, potentialPodName(metadata))
				inject = true
				useDefault = false
				break
	...
	default: // InjectionPolicyOff
		istiolog.Errorf("Illegal value for autoInject:%s, must be one of [%s,%s]. Auto injection disabled!",
	...
	if istiolog.DebugEnabled() {
		// Build a istiolog message for the annotations.
		annotationStr := ""
		...
		istiolog.Debugf("Sidecar injection policy for %v/%v: namespacePolicy:%v useDefault:%v inject:%v required:%v %s",
			metadata.Namespace,
...
func InjectionData(sidecarTemplate, valuesConfig, version string, typeMetadata *metav1.TypeMeta, deploymentMetadata *metav1.ObjectMeta, spec *corev1.PodSpec,
	...
	*SidecarInjectionSpec, string, error) {
	...
	if spec.DNSPolicy != corev1.DNSClusterFirst {
		istiolog.Warnf("%q's DNSPolicy is not %q. The Envoy sidecar may not able to connect to Istio Pilot",
			metadata.Namespace+"/"+metadata.Name, corev1.DNSClusterFirst)
	}

	if err := validateAnnotations(metadata.GetAnnotations()); err != nil {
		istiolog.Errorf("Injection failed due to invalid annotations: %v", err)
		return nil, "", err
	}

	values := map[string]interface{}{}
	if err := yaml.Unmarshal([]byte(valuesConfig), &values); err != nil {
		istiolog.Infof("Failed to parse values config: %v [%v]\n", err, valuesConfig)
	...
	if err := yaml.Unmarshal(bbuf.Bytes(), &sic); err != nil {
		...
		istiolog.Warnf("Failed to unmarshal template %v %s", err, bbuf.String())
...
func parseTemplate(tmplStr string, funcMap map[string]interface{}, data SidecarTemplateData) (bytes.Buffer, error) {
	...
	if err != nil {
		istiolog.Infof("Failed to parse template: %v %v\n", err, tmplStr)
		return bytes.Buffer{}, err
	}
	if err := t.Execute(&tmpl, &data); err != nil {
		istiolog.Infof("Invalid template: %v %v\n", err, tmplStr)
		return bytes.Buffer{}, err
	}
...
func structToJSON(v interface{}) string {
	...
	if err != nil {
		istiolog.Warnf("Unable to marshal %v", v)
		return "{}"
	}
...
func toJSON(m map[string]string) string {
	...
	if err != nil {
		istiolog.Warnf("Unable to marshal %v", m)
		return "{}"
	}
...
func fromJSON(j string) interface{} {
	...
	if err != nil {
		istiolog.Warnf("Unable to unmarshal %s", j)
		return "{}"
	}
...
func toYaml(value interface{}) string {
	...
	if err != nil {
		istiolog.Warnf("Unable to marshal %v", value)
		return ""
	}
