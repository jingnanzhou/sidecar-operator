...
func extractStatusPort(sidecar *corev1.Container) int {
	...
	logger := log.WithField("context", "extractConcurrency")
	...
	for i, arg := range sidecar.Args {
		...
		if portStr == "" {
			...
			if len(sidecar.Args) <= i+1 {
				logger.Errorf("No statusPort value provided, skip app probe rewriting")
				return -1
			}
		...
		if err != nil {
			logger.Errorf("Failed to convert statusPort to int %v, err %v", portStr, err)
			return -1
		}
		return p
	}
...
func DumpAppProbers(podspec *corev1.PodSpec) string {
	logger := log.WithField("context", "DumpAppProbers")
	...
	if err != nil {
		logger.Errorf("failed to serialize the app prober config %v", err)
		return ""
	}
	return string(b)
}
...
//	logger := log.WithField("context", "rewriteAppHTTPProbe")
